...
def gen_yaml_input_files(height_range, period_range, diameter_uc_range, diameter_bc_range, diameter_mc_range, diameter_p_range, run_folder='deepcwind_cases', template="OC4semi-RAFT_QTF.yaml"):
    ...
    for i in range(len(height_range)):
       ...
       # main column:
        ...
        yaml.dump(tfile, open(run_folder+'/OC6semi-RAFT_h_'+ str(height_range[i]) + '_T_' + str(period_range[i]) + '_dmc_' + str(diameter_mc_range[i]) +
...
if __name__=="__main__":
    ...
    df = pd.read_csv('shreyas_data.csv')  # Replace with your filename
...
#    diameter_p = [1.28]
    ...
    gen_yaml_input_files(height_range=wave_height_range, period_range=wave_period_range, diameter_uc_range=diameter_uc, diameter_bc_range=diameter_bc, diameter_mc_range=diameter_mc, diameter_p_range=diameter_p, run_folder='OC6semi_cases', template="OC6semi-RAFT_wamit.yaml")
